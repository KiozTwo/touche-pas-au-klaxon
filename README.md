...
- Administrateur : liste des utilisateurs en lecture seule (jeu de données des employés), liste complète des trajets et gestion des agences. Une agence utilisée par un trajet ne peut pas être supprimée (clé étrangère).
...
`public/index.php` : point d'entrée et routage ; `src/Controllers/` : actions HTTP et autorisations ; `src/Repositories/` : accès PDO ; `src/Support/` : connexion et règles métier ; `views/` : présentations ; `sql/` : schéma, données initiales et fichiers sources ; `docs/` : MCD et MLD.
...
Dépôt du projet : https://github.com/KiozTwo/touche-pas-au-klaxon. Le livrable PDF remis séparément contient les identifiants de démonstration.
