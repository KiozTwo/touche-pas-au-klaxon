...
work_dir="$(mktemp -d)"
server_pid=''
cleanup() { if [[ -n "$server_pid" ]]; then kill "$server_pid" 2>/dev/null || true; fi; rm -rf "$work_dir"; }
trap cleanup EXIT
...
[[ "$count" == 1 ]]
mysql -h 127.0.0.1 -u root -proot klaxon_test -e "INSERT INTO trips(departure_agency_id,arrival_agency_id,departure_at,arrival_at,total_seats,available_seats,author_id) VALUES(1,2,NOW()+INTERVAL 3 DAY,NOW()+INTERVAL 3 DAY+INTERVAL 2 HOUR,4,3,(SELECT id FROM users WHERE email='other@example.test'));"
